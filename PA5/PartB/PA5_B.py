...
# Implement perspective projection. For each point, project it along the projection direction to into the plane.
primepoints = []
for point in range(len(pointsList)):

    perspectiveDirection = np.negative(point)
    t = (np.divide(np.dot((np.subtract(plane.getpoint(),pointsList[point].getpoint())),plane.getNormal()),np.dot(pointsList[point].getpoint(),plane.getNormal())))
    primepoint = np.add(pointsList[point].getpoint(),np.multiply(t,pointsList[point].getpoint()))
